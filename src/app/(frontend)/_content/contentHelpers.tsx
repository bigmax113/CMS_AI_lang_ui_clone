...
const articleViewCount = (article: Pick<Article, 'id'> & { viewCount?: null | number }) => {
  if (typeof article.viewCount !== 'number' || !Number.isFinite(article.viewCount)) {
    return defaultArticleViewCount
  }

  const viewCount = Math.max(0, Math.round(article.viewCount))
...
const localizedUIFallbacks: Partial<Record<ArticleLanguageCode, Partial<Record<FrontendUIKey, string>>>> = {
  bg: {
    ...
    'nav.products.pc': 'PC',
    'nav.products.racingAccessories': 'Racing ?????????',
    'nav.products.racingCockpits': 'Racing ???????',
  ...
  cs: {
    ...
    'footer.warranty': 'Z?ru?n? podm?nky a z?ru?n? karty',
    ...
    'nav.products.pc': 'PC',
    'nav.products.racingAccessories': 'Racing p??slu?enstv?',
    'nav.products.racingCockpits': 'Racing kokpity',
    'nav.products.webcams': 'Webkamery',
    'nav.searchInputLabel': 'Hledat v ?l?nc?ch',
    'nav.searchLabel': 'Otev??t vyhled?v?n?',
    'nav.searchPlaceholder': 'Hledat',
    'nav.searchSubmit': 'Hledat',
    'nav.solutions': '?e?en?',
    'nav.solutions.pcGaming': 'PC hern? ?e?en?',
    'nav.solutions.simRacingFlex': 'Sim Racing Flex ?e?en?',
    'nav.solutions.simRacingPro': 'Sim Racing Pro ?e?en?',
    'nav.solutions.streaming': 'Streamovac? ?e?en?',
    'nav.whereToBuy': 'Kde koupit',
    'subscribe.aria': 'Odb?r blogu',
    'subscribe.description': 'Dost?vejte nejnov?j?? novinky a post?ehy p??mo do sv? schr?nky.',
    'subscribe.emailAriaLabel': 'E-mail pro odb?r',
    'subscribe.placeholder': 'Zadejte sv?j e-mail',
    'subscribe.saving': 'Ukl?d?n?...',
    'subscribe.submit': 'Odeb?rat',
    'subscribe.title': 'Odeb?rejte n?? blog',
  },
  uk: {
    'article.likeAriaLabel': '????????? ??????',
    'article.popularNews': '????????? ??????',
    ...
    'nav.searchSubmit': '??????',
    'nav.solutions': '???????',
    'nav.solutions.pcGaming': 'PC gaming ???????',
    'nav.solutions.simRacingFlex': 'Sim Racing Flex ???????',
    'nav.solutions.simRacingPro': 'Sim Racing Pro ???????',
    'nav.solutions.streaming': '??????????? ???????',
    'nav.whereToBuy': '?? ??????',
    'subscribe.aria': '???????? ?? ????',
    'subscribe.description': '????????? ??????? ?????? ?? ??????? ????? ?? ?????.',
    'subscribe.emailAriaLabel': 'E-mail ??? ????????',
...
const stableLocalizedUIFallbacks: Partial<Record<ArticleLanguageCode, Partial<Record<FrontendUIKey, string>>>> = {
  bg: {
    'article.likeAriaLabel': '??????? ????????',
    'article.popularNews': '????????? ??????',
    'article.reactionsAriaLabel': '??????? ??? ????????',
  ...
  cs: {
    ...
    'article.shareAriaLabel': 'Sd?let ?l?nek',
    'article.shareLabel': 'Sd?let:',
    'article.sidebarAria': 'Postrann? panel ?l?nku',
    'article.tagsLabel': 'T?mata:',
    'blog.articlesAria': '?l?nky',
    'blog.clearFilters': 'Vymazat v?echny filtry',
    ...
    'blog.popularNews': 'Popul?rn? novinky',
    'blog.previousPage': 'P?edchoz? str?nka',
    'blog.readMore': '??st d?le',
    'blog.topicsTitle': 'T?mata',
    'blog.viewsSuffix': 'zobrazen?',
    'cta.aria': 'Kontaktovat LORGAR',
    'cta.becomePartner': 'STA?TE SE PARTNEREM',
    'cta.contactUs': 'KONTAKTUJTE N?S',
    'cta.title': 'Chcete spolupracovat s LORGAR?',
    'footer.about': 'O LORGAR',
    'footer.aria': 'Pati?ka',
    'footer.cookies': 'Z?SADY COOKIES',
    'footer.copyright': '? LORGAR 2026. V?ECHNA PR?VA VYHRAZENA',
    ...
    'nav.products.webcams': 'Webkamery',
    'nav.searchInputLabel': 'Hledat v ?l?nc?ch',
    'nav.searchLabel': 'Hledat',
    'nav.searchPlaceholder': 'Hledat',
    'nav.searchSubmit': 'Hledat',
    'nav.solutions': '?E?EN?',
    'nav.solutions.pcGaming': 'PC Gaming ?e?en?',
    'nav.solutions.simRacingFlex': 'Sim Racing Flex ?e?en?',
    'nav.solutions.simRacingPro': 'Sim Racing Pro ?e?en?',
    'nav.solutions.streaming': 'Streamovac? ?e?en?',
    'nav.whereToBuy': 'KDE KOUPIT',
    'subscribe.aria': 'Odb?r blogu',
    'subscribe.description': 'Z?skejte nejnov?j?? novinky a post?ehy p??mo do sv? schr?nky.',
...
const uiLabel = (uiStrings: LorgarUIStrings, key: FrontendUIKey, languageCode?: null | string) => {
  ...
  return isBrokenLocalizedLabel(translated) ? defaultFrontendUIStrings[key] : translated
}

const translatedLinkLabel = (item: LorgarTranslatedLink, uiStrings: LorgarUIStrings, languageCode?: null | string) =>
  item.translationKey ? uiLabel(uiStrings, item.translationKey, languageCode) : item.label
const localizedTopicLabelKeys: Partial<Record<string, FrontendUIKey>> = {
  all: 'tag.all',
  article: 'tag.article',
  'astral esports': 'tag.astralEsports',
  'blog post': 'tag.blogPost',
  chairs: 'tag.chairs',
  ...
  'gaming chairs': 'tag.gamingChairs',
  'gaming devices': 'tag.gamingDevices',
  'gaming tournament': 'tag.gamingTournament',
  innovation: 'tag.innovation',
  news: 'tag.news',
  partners: 'tag.partners',
  partnerships: 'tag.partnerships',
  passion: 'tag.passion',
  'performance lab': 'tag.performanceLab',
  platform: 'tag.platform',
  'product content': 'tag.productContent',
  products: 'tag.products',
  'rainbow six siege': 'tag.rainbowSixSiege',
  rankings: 'tag.rankings',
  'release note': 'tag.releaseNote',
  'stake ranked': 'tag.stakeRanked',
}
...
const LorgarExclusiveMenuScript = () => (
  ...
  const hoverQuery = window.matchMedia ? window.matchMedia('(hover: hover) and (pointer: fine)') : { matches: false };
  ...
  const openMenu = (details) => {
    const root = details.closest('.lorgar-header');
    if (!root) return;
    closeMenus(root, details);
    details.open = true;
  };
  ...
  const wireMenu = (details) => {
    if (details.dataset.lorgarHoverWired) return;
    details.dataset.lorgarHoverWired = '1';
  ...
  const wireAll = () => document.querySelectorAll('details[data-lorgar-exclusive-menu]').forEach(wireMenu);
  wireAll();
  document.addEventListener('click', (event) => {
    const summary = event.target instanceof Element ? event.target.closest('.lorgar-nav-dropdown > summary') : null;
    if (!summary || !hoverQuery.matches) return;
    event.preventDefault();
    const details = summary.closest('details');
    if (details) openMenu(details);
  }, true);
  document.addEventListener('toggle', (event) => {
    ...
    if (root) closeMenus(root, target);
  }, true);
...
export const LorgarArticlesIndexLayout = ({
  ...
  const hiddenTopicFilters = new Set(['all', 'news'])
  ...
  const sectionHeading = isPopularSort ? uiLabel(uiStrings, 'blog.popularNews') : uiLabel(uiStrings, 'blog.latestNews')
              ...
              <div className="lorgar-blog-topics__items">
                ...
                {selectedTopicQueries.length ? (
                  <a className="lorgar-blog-topics__clear" href={clearTopicsHref}>
                    {uiLabel(uiStrings, 'blog.clearFilters')}
                  </a>
                ) : null}
              </div>
            </nav>
...
const LorgarMetaAuthorIcon = () => (
  <svg aria-hidden="true" className="lorgar-meta__icon lorgar-meta__icon--author" fill="none" viewBox="0 0 24 24">
              ...
              <nav aria-label="Article tags" className="lorgar-article-tags">
                ...
                {articleTags.map((tag) => (
                  <Link href={lorgarArticlesPath({ languageCode: article.languageCode, tagQuery: tag })} key={tag} prefetch={false}>
                    {localizedTagLabel(tag, uiStrings, article.languageCode)}
                  </Link>
                ))}
              </nav>
            ) : null}
